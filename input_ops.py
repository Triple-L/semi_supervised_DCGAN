...
def create_input_ops(dataset,
                     batch_size,
                     num_threads=16, #16          # for creating batches
                     is_training=False,
                     data_id=None,
                     scope='inputs',
                     shuffle=True,
    ...
    # single operations
    with tf.device("/gpu:2"), tf.name_scope(scope):
    ...
    # batchify
    capacity = 2 * batch_size * num_threads #  capacity = 2 * batch_size * num_threads
    min_capacity = min(int(capacity * 0.75), 1024)#min(int(capacity * 0.75), 1024)
